<template>
    <div class="vx-auto-suggest">
        <div class="flex items-center relative">
            <vs-input :placeholder="placeholder" ref="input" :class="inputClassses" class="z-50" icon-pack='feather'
                      icon='icon-search' icon-no-border v-model="searchQuery" @keyup.esc="escPressed"
            ...
            searchQuery(val) {
                if (val == '') {
                    ...
                    let exactEle = this.data.data.filter((item) => {
                        return item.public_id.toLowerCase().startsWith(this.searchQuery.toLowerCase())
                    });
                    let containEle = this.data.data.filter((item) => {
                        return !item.public_id.toLowerCase().startsWith(this.searchQuery.toLowerCase()) && item.public_id.toLowerCase().indexOf(this.searchQuery.toLowerCase()) > -1
                    });
